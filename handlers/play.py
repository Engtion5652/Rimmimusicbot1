...
async def play(_, message: Message):
    global que
    lel = await message.reply("🔄 **Processing Via 👉 @EsportCheater**")
                      ...
                      if administrator == message.from_user.id:  
                          ...
                          try:
                              ...
                              await lel.edit(
                                  ...
                                  "\n\nOr manually add @SankiPublicEnjoy to your Group and try again</b>",
                              )
                              pass
    try:
        chatdetails = await USER.get_chat(chid)
    ...
    await lel.edit("🔎 **Finding Via 👉 @EsportCheater**")
    sender_id = message.from_user.id
    ...
    for i in message.command[1:]:
        query += ' ' + str(i)
    print(query)
    await lel.edit("🎵 **Processing Via 👉 @EsportCheater**")
...
async def deezer(client: Client, message_: Message):
    global que
    lel = await message_.reply("🔄 **Processing Via 👉 @EsportCheater**")
    ...
    if message_.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        appendable = [s_name, r_by, loc]
        qeue.append(appendable)
        await res.edit_text(f"👉 @EsportCheater= #️⃣ Queued at position {position}")
    else:
        await res.edit_text("👉 @EsportCheater=▶️ Playing.....")
...
async def jiosaavn(client: Client, message_: Message):
    ...
    if message_.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        m = await client.send_photo(
            ...
            reply_markup=keyboard,
            photo="final.png",
            caption=f"👉 @EsportCheater=#️⃣ Queued at position {position}",
